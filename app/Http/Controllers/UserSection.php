...
use App\Http\Middleware\Frontuser as MiddlewareFrontuser;
...
class UserSection extends Controller
{
    ...
    public function view_Admins(Request $request)
    {
        ...
        return view('Back-End.Users.view_admins', compact('userData'));
    }
    public function view_users(Request $request)
    {
        // get data from users table

        $userData = FrontUser::where(['status' => 1])->get();
        return view('Back-End.Users.view_front_users', compact('userData'));
    }
    ...
    // delete the Front user
    public function Delete_Front_user($id)
    {
        if (empty($id)) {
            Toastr::error('The User is Not registered yet', 'Error');
            return back();
